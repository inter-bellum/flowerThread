...
void Module::clearLEDS(){
  strip->clear();
  strip->show();
}

uint16_t Module::get(uint8_t index){
  return this->potValues[i];
}

uint8_t Module::getPin(uint8_t i){
...
void Module::interpolateColorSpace(float xIn, float yIn, float zIn, uint8_t* colors){
  float x = xIn / 4096.;
  float y = yIn / 819.2;
  float z = zIn / 4096.;
  int floorY = floor(y);
